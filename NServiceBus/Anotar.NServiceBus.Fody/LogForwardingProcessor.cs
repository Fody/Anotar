...
public class LogForwardingProcessor
{
    ...
    void ProcessInstruction(Instruction instruction)
    {
        ...
        if (methodReference.DeclaringType.FullName != "Anotar.NServiceBus.LogTo")
        {
            return;
        }
        if (!foundUsageInMethod)
        ...
        if (parameters.Count == 0)
        {
            instructions.Replace(instruction,
                new[]
                {
                    ...
                    Instruction.Create(OpCodes.Callvirt, ModuleWeaver.GetNormalFormatOperand(methodReference))
                });
            return;
        }
