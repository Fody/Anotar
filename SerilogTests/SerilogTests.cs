...
public class SerilogTests
{
    ...
    public SerilogTests()
    {
		...
		beforeAssemblyPath = Path.GetFullPath(@"..\..\..\AssemblyToProcess\bin\DebugSerilog\SerilogAssemblyToProcess.dll");
#if (!DEBUG)
    ...
    public void TraceStringParams()
    {
		...
		//Assert.IsTrue(Infos.First().StartsWith("Method: 'System.Void ClassWithLogging::TraceStringParams()'. Line: ~"));
    }
    ...
    public void TraceStringException()
    {
		...
		Assert.AreEqual(1, Infos.Count);
	//	Assert.IsTrue(Infos.First().StartsWith("Method: 'System.Void ClassWithLogging::TraceStringException()'. Line: ~"));
    }
